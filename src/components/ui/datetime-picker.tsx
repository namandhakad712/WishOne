...
// DateTimePicker Demo for GoalsPage
export function DateTimePicker({ value, onChange }: { value: any, onChange: (date: any) => void }) {
  const [date, setDate] = React.useState<Date>();
  const [open, setOpen] = React.useState(false);

  React.useEffect(() => {
    if (value instanceof Date) setDate(value);
    else if (typeof value === "string" && value) setDate(new Date(value));
    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverContent className="w-auto p-0">
        ...
        <div className="p-3 border-t border-border">
          <TimePicker setDate={d => { setDate(d); onChange(d); }} date={date} />
          <Button
            type="button"
            size={'sm'}
            className="mt-3 w-full bg-gradient-to-r from-indigo-400 via-purple-400 to-blue-400 shadow-xl border border-gray-200 dark:border-zinc-700 backdrop-blur-md rounded-xl text-white font-semibold transition-all hover:from-indigo-500 hover:to-blue-500 focus:ring-2 focus:ring-indigo-400 focus:outline-none"
            onClick={() => {
              const now = new Date();
              setDate(now);
              onChange(now);
